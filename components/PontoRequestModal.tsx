...
interface PontoRequestModalProps {
  isOpen: boolean;
  onClose: () => void;
...
const PontoRequestModal: React.FC<PontoRequestModalProps> = ({
  isOpen,
  onClose,
  onSubmit,
  academicPeriods,
  ...
  return (
    <div
      role="dialog"
      aria-modal="true"
      aria-labelledby="ponto-request-modal-title"
    ...
    >
      <div className="bg-white rounded-lg shadow-xl p-6 sm:p-8 w-full max-w-lg transform transition-all">
        <div className="flex justify-between items-center mb-6">
          <h2 id="ponto-request-modal-title" className="text-xl sm:text-2xl font-semibold text-gray-800">
            Solicitar Registro de Ponto/Atividades
          </h2>
          <button
            onClick={onClose}
        ...
        <div className="space-y-4">
          <p className="text-sm text-gray-700">
            Selecione o(s) período(s) acadêmico(s) e forneça uma justificativa para solicitar o Registro de Ponto ou Relatório de Atividades Docentes.
          </p>
            ...
            {academicPeriods.length > 0 ? (
              academicPeriods.map((period) => (
                <label
                  ...
                  htmlFor={`modal-ponto-period-${period}`}
                >
                  <input
                    type="checkbox"
                    id={`modal-ponto-period-${period}`}
          ...
          <div>
            <label htmlFor="ponto-justification" className="block text-sm font-medium text-gray-700 mb-1">
              Justificativa <span className="text-red-500">*</span>
            </label>
            <textarea
              id="ponto-justification"
              name="justification"
              rows={3}
              value={justification}
              ...
              required
              aria-describedby={error && (!justification.trim() || selectedPeriods.length === 0) ? "ponto-error-message" : undefined}
            />
          </div>

          {error && (
             <p id="ponto-error-message" role="alert" className="text-xs text-red-600">{error}</p>
          )}
        </div>
...
export default PontoRequestModal;